...
<div class="mb-6 p-5 bg-white rounded-2xl shadow-sm border border-slate-200">
    <form action="{{ route('provinsis.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end" id="searchForm">
        ...
        @if(request('search_id'))
            <a href="{{ route('provinsis.index') }}" class="bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-300 transition-colors">
                Reset Filter
            </a>
        @endif
    </form>
        ...
        <table class="w-full text-sm text-left text-slate-700 whitespace-nowrap">
            <thead class="text-xs text-slate-600 uppercase bg-slate-100 border-b border-slate-200 tracking-wider">
                <tr>
                    ...
                    <th class="px-6 py-4 font-semibold">Nama Provinsi</th>
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr class="bg-white border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4 font-medium">{{ $provinsi->id }}</td>
