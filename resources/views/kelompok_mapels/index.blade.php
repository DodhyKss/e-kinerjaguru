...
<div class="mb-6 p-5 bg-white rounded-2xl shadow-sm border border-slate-200">
    <form action="{{ route('kelompok-mapels.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end" id="searchForm">
            ...
            <select name="search_id" class="select2-filter w-full" onchange="document.getElementById('searchForm').submit()">
                ...
                @foreach($allData as $item)
                    <option value="{{ $item->id }}" {{ request('search_id') == $item->id ? 'selected' : '' }}>
                        {{ $item->nama_kelompok_mapel }}
                    </option>
                @endforeach
            </select>
        </div>
        @if(request('search_id'))
            <a href="{{ route('kelompok-mapels.index') }}" class="bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-300 transition-colors">
                Reset Filter
            </a>
        @endif
    </form>
