@extends('layouts.app')
@section('title', 'Edit Kelompok Mapel')
...
<div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="px-8 py-6 border-b border-slate-100 bg-slate-50">
        <h2 class="text-xl font-bold text-slate-800">Form Edit Kelompok Mapel</h2>
    </div>
    
    <div class="p-8">
        <form action="{{ route('kelompok-mapels.update', $kelompokMapel) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" name="nama_kelompok_mapel" id="nama_kelompok_mapel" value="{{ old('nama_kelompok_mapel', $kelompokMapel->nama_kelompok_mapel) }}" required
