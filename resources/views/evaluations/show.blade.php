...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 h-full relative overflow-hidden">
            <div class="absolute top-0 right-0 p-6">
                @if($evaluation->status == 'approved')
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 border border-emerald-200">Disetujui Kepala Sekolah</span>
                @elseif($evaluation->status == 'completed')
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200">Menunggu Review Kepala Sekolah</span>
                @elseif($evaluation->status == 'in_progress')
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800 border border-amber-200">Proses Penilaian</span>
                @else
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-800 border border-slate-200">Draft / Belum Dimulai</span>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                <div>
                    ...
                    <p class="text-xs text-slate-600 mt-1">Pangkat/Golongan: <span class="font-medium text-slate-800">{{ $evaluation->guru->pangkatGolongan->nama ?? '-' }} {{ isset($evaluation->guru->pangkatGolongan->golongan) ? '('.$evaluation->guru->pangkatGolongan->golongan.')' : '' }}</span></p>
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-600 mt-1">Pangkat/Golongan: <span class="font-medium text-slate-800">{{ $evaluation->penilai->pangkatGolongan->nama ?? '-' }} {{ isset($evaluation->penilai->pangkatGolongan->golongan) ? '('.$evaluation->penilai->pangkatGolongan->golongan.')' : '' }}</span></p>
    ...
    <div>
        <div class="bg-gradient-to-br from-indigo-500 to-violet-600 rounded-2xl p-6 shadow-md border border-indigo-400 h-full text-white flex flex-col justify-between">
            <div>
            ...
            <div class="mt-6 pt-6 border-t border-indigo-400/30">
                ...
                <div class="flex items-center gap-3">
                    <div class="w-full bg-indigo-900/40 rounded-full h-2">
                        <div class="bg-white h-2 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.5)]" style="width: {{ $evaluation->progress }}%"></div>
                    </div>
...
<div class="bg-blue-50 border border-blue-100 rounded-2xl p-6 mb-8">
    <div class="flex items-start gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-blue-800 mt-2 leading-relaxed whitespace-pre-wrap">{{ $evaluation->catatan_kepala_sekolah }}</p>
        </div>
    </div>
</div>
@endif
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8">
    ...
    <form action="{{ route('evaluations.approve', $evaluation) }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="catatan_kepala_sekolah" class="block text-sm font-medium text-slate-700 mb-2">Catatan / Feedback (Opsional)</label>
            <textarea name="catatan_kepala_sekolah" id="catatan_kepala_sekolah" rows="3" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 border" placeholder="Berikan catatan mengenai hasil evaluasi ini..."></textarea>
        </div>
        <button type="submit" class="bg-emerald-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors flex items-center shadow-sm">
...
@if($canEvaluate && in_array($evaluation->status, ['in_progress', 'draft']))
    ...
    <div class="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-emerald-700 mt-1">Seluruh butir indikator telah dinilai. Anda dapat mengirimkan hasil evaluasi ini ke Kepala Sekolah untuk direview.</p>
        </div>
        <form action="{{ route('evaluations.submit', $evaluation) }}" method="POST">
            @csrf
            <button type="submit" class="bg-emerald-600 text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-emerald-700 transition-colors shadow-sm flex items-center whitespace-nowrap">
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    <div class="flex-1">
                        ...
                        <div class="mt-4 flex gap-2">
                            @if($ind->has_observasi) <span class="inline-flex items-center text-[11px] font-medium text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded"><i data-lucide="eye" class="w-3 h-3 mr-1"></i> Observasi</span> @endif
                            @if($ind->has_telaah_dokumen) <span class="inline-flex items-center text-[11px] font-medium text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded"><i data-lucide="file-text" class="w-3 h-3 mr-1"></i> Telaah Dokumen</span> @endif
                            @if($ind->has_wawancara) <span class="inline-flex items-center text-[11px] font-medium text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded"><i data-lucide="mic" class="w-3 h-3 mr-1"></i> Wawancara</span> @endif
                        </div>
                    </div>
                    
                    <div class="md:w-64 flex flex-col justify-center border-l md:border-l-slate-100 border-t md:border-t-transparent pt-4 md:pt-0 pl-0 md:pl-6">
                        @if($isCompleted)
                            <div class="mb-3">
                                <span class="text-xs text-slate-500 mb-1 block">Level Capaian:</span>
                                <div class="inline-flex items-center px-3 py-1 rounded-lg font-bold text-sm bg-indigo-50 text-indigo-700 border border-indigo-100">
                        ...
                        @else
                            ...
                            @if(auth()->user()->isGuru() && $ind->has_telaah_dokumen && in_array($evaluation->status, ['draft', 'in_progress']))
                                <a href="{{ route('evaluations.upload', [$evaluation, $ind]) }}" class="inline-flex items-center justify-center w-full px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-200 text-sm font-medium rounded-xl hover:bg-indigo-100 transition-colors shadow-sm">
