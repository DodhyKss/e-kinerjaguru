...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden max-w-3xl">
    <div class="px-8 py-6 border-b border-slate-100 flex items-start gap-4 bg-indigo-50/30">
        ...
        <div>
            <h3 class="text-lg font-bold text-slate-900">Form Tambah Periode</h3>
            <p class="text-sm text-slate-600 mt-1">Buat jadwal periode baru untuk pelaksanaan evaluasi kinerja.</p>
        </div>
    </div>
    
    <form action="{{ route('evaluation-periods.store') }}" method="POST" class="p-8">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('nama') border-rose-300 ring-rose-500 @enderror" placeholder="Contoh: Evaluasi Kinerja Guru Semester Ganjil 2025/2026">
            ...
            <div>
                ...
                <input type="text" name="tahun_ajaran" id="tahun_ajaran" value="{{ old('tahun_ajaran') }}" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('tahun_ajaran') border-rose-300 ring-rose-500 @enderror" placeholder="Contoh: 2025/2026">
            ...
            <div>
                ...
                <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai') }}" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('tanggal_mulai') border-rose-300 ring-rose-500 @enderror">
            ...
            <div>
                ...
                <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ old('tanggal_selesai') }}" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('tanggal_selesai') border-rose-300 ring-rose-500 @enderror">
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-sm transition-colors flex items-center">
                <i data-lucide="save" class="w-4 h-4 mr-2"></i> Simpan Periode
            </button>
        </div>
    </form>
</div>
