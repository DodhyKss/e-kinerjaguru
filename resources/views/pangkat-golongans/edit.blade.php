...
@extends('layouts.app')
@section('title', 'Edit Pangkat / Golongan')

@section('content')
<div class="mb-6">
    <a href="{{ route('pangkat-golongans.index') }}" class="inline-flex items-center text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors">
...
<div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="px-8 py-6 border-b border-slate-100 bg-slate-50">
        <h2 class="text-xl font-bold text-slate-800">Form Edit Pangkat / Golongan</h2>
    </div>
    
    <div class="p-8">
        <form action="{{ route('pangkat-golongans.update', $model) }}" method="POST">
            @csrf
            @method('PUT')
            
            <div class="mb-6">
                <label for="nama" class="block text-sm font-semibold text-slate-700 mb-2">Nama Pangkat / Golongan <span class="text-red-500">*</span></label>
