...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden max-w-3xl">
    <div class="px-8 py-6 border-b border-slate-100">
        <h3 class="text-lg font-bold text-slate-900">Form Edit Sekolah</h3>
        <p class="text-sm text-slate-500 mt-1">Perbarui data detail sekolah: <span class="font-semibold text-indigo-600">{{ $school->nama }}</span></p>
    </div>
    
    <form action="{{ route('schools.update', $school) }}" method="POST" class="p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $school->nama) }}" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('nama') border-rose-300 ring-rose-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="npsn" id="npsn" value="{{ old('npsn', $school->npsn) }}" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('npsn') border-rose-300 ring-rose-500 @enderror">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="alamat" id="alamat" rows="3" required class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border @error('alamat') border-rose-300 ring-rose-500 @enderror">{{ old('alamat', $school->alamat) }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="telepon" id="telepon" value="{{ old('telepon', $school->telepon) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border">
            </div>

            <div>
                <label for="email" class="block text-sm font-bold text-slate-700 mb-1">Email Sekolah</label>
                <input type="email" name="email" id="email" value="{{ old('email', $school->email) }}" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2.5 border">
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
            <a href="{{ route('schools.index') }}" class="px-5 py-2.5 text-sm font-medium text-slate-600 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-colors">Batal</a>
