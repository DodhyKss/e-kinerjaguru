...
<div class="mb-6 flex justify-between items-center">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Edit Jenis Dokumen</h2>
            <p class="text-sm text-slate-500 mt-1">Ubah nama jenis dokumen dan perbarui pemetaannya.</p>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <div class="lg:col-span-2">
        <form action="{{ route('jenis-dokumens.update', $jenisDokumen) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-8 border-b border-slate-100">
                <div class="mb-6">
                    ...
                    <input type="text" name="nama_jenis_dokumen" id="nama_jenis_dokumen" value="{{ old('nama_jenis_dokumen', $jenisDokumen->nama_jenis_dokumen) }}" class="w-full text-sm border-slate-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-4 py-3 bg-slate-50" required>
            ...
            <div class="p-8 bg-slate-50/50">
                ...
                <p class="text-xs text-slate-500 mb-6 leading-relaxed">
                    Centang aspek penilaian di bawah ini yang dapat dibuktikan oleh dokumen ini.
                </p>
                    ...
                    @forelse($aspects as $indicatorId => $indicatorAspects)
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                            ...
                            <div class="space-y-3">
                                ...
                                @php 
                                    $isChecked = in_array($aspect->id, old('aspects', $selectedAspects)); 
                                    $isMappedToOther = $aspect->jenis_dokumen_id && $aspect->jenis_dokumen_id !== $jenisDokumen->id;
                                @endphp
                                <label class="flex items-start gap-3 cursor-pointer group p-2 hover:bg-slate-50 rounded-lg transition-colors">
                                    <div class="flex items-center h-5 mt-0.5">
                                        <input type="checkbox" name="aspects[]" value="{{ $aspect->id }}" {{ $isChecked ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500">
                                    </div>
                                    <div class="flex-1 text-sm">
                                        <span class="font-medium {{ $isChecked ? 'text-indigo-700' : 'text-slate-700 group-hover:text-slate-900' }} block">{{ $aspect->aspek }}</span>
                                        @if($isMappedToOther)
            ...
            <div class="p-6 border-t border-slate-100 bg-white flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-colors shadow-sm">Simpan Perubahan</button>
            </div>
        </form>
    </div>
